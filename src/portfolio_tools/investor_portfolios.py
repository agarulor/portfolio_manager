...
from portfolio_tools.risk_metrics import portfolio_volatility, calculate_max_drawdown, calculate_covariance, \
    max_drawdown_from_value_series
...
def add_risk_free_column(returns: pd.DataFrame,
    ...
    Returns
    -------
    pd.DataFrame: add risk free column output.
    """
...
def add_risk_free_asset(
    ...
    Returns
    -------
    Tuple[pd.DataFrame, np.ndarray]: add risk free asset output.
    """
    returns_ext = add_risk_free_column(returns, rf_annual, periods_per_year)
...
def get_investor_weights(returns: pd.DataFrame,
    ...
    Returns
    -------
    np.ndarray: get investor weights output.
    """
...
def get_results(returns: pd.DataFrame,
    ...
    returns : pd.DataFrame. Returns of the assets.
    covmat : pd.DataFrame. covmat.
    weights : pd.DataFrame. Portfolio weights allocated to each asset.
    method : Literal["simple", "log"]. method.
    periods_per_year : float. periods per year.
    rf_annual : float|None. Annual risk-free rate.

    Returns
    -------
    dict: get results output.
    """
...
def get_investor_initial_portfolio(returns: pd.DataFrame,
    ...
    """
    Gets investor initial portfolio.

    Parameters
    ----------
    returns : pd.DataFrame. Returns of the assets.
    method : Literal["simple", "log"]. method.
    portfolio_type : Literal["investor", "msr", "gmv", "ew", "random"]. portfolio type.
    periods_per_year : float. periods per year.
    min_w : float. min w.
    max_w : float. max w.
    rf_annual : float | None. Annual risk-free rate.
    ...
    Returns
    -------
    pd.DataFrame: get investor initial portfolio output.
    """
...
def get_cumulative_returns(returns: pd.DataFrame,
    ...
    ----------
    returns : pd.DataFrame. Returns of the assets.
    weights : Optional[np.ndarray]. Portfolio weights allocated to each asset.
...
def get_sector_exposure_table(
    ...
    """
    Gets sector exposure table.

    Parameters
    ----------
    df_weights : pd.DataFrame. df weights.
    sectors : pd.DataFrame. sectors.
    weight_col : str. weight col.
    as_percent : bool. as percent.

    Returns
    -------
    pd.DataFrame: get sector exposure table output.
    """
...
def create_output_table_portfolios(returns: pd.DataFrame,
    ...
    """
    Creates output table portfolios.

    Parameters
    ----------
    returns : pd.DataFrame. Returns of the assets.
    method : Literal["simple", "log"]. method.
    list_portfolio_types : Any. list portfolio types.
    periods_per_year : float. periods per year.
    min_w : float. min w.
    max_w : float. max w.
    rf_annual : float | None. Annual risk-free rate.
    ...
    # we concatenate the results in a new df
...
def render_historical_portfolios_results(returns: pd.DataFrame,
    ...
    """
    Renders historical portfolios results.

    Parameters
    ----------
    returns : pd.DataFrame. Returns of the assets.
    initial_investment : float. initial investment.
    weights : Any. Portfolio weights allocated to each asset.
    list_portfolio_types : Any. list portfolio types.
    periods_per_year : float. periods per year.
    rf_annual : float. Annual risk-free rate.
